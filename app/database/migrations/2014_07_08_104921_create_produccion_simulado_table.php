...
class CreateProduccionSimuladoTable extends Migration {
	...
	public function up()
	{
		...
		Schema::create('produccion_simulado', function(Blueprint $table)
		{
			//
			$table->increments('id');
			$table->date('date');
			...
			$table->decimal('FCR', 10,2);
            $table->decimal('SFR',10,2);
            $table->integer('cantidad_toma_modelo');
            $table->integer('cantidad_toma');
			$table->timestamps();
		});
	...
	{
		Schema::drop('produccion_simulado', function(Blueprint $table)
		{
			//
		});
	}
